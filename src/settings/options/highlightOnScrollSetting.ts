...
export default class HighlightOnScrollSetting extends DynamicOutlineSetting {
	public display(): void {
		new Setting(this.containerEl)
			...
			.setDesc(
				htmlDescription(
					`When scrolling throught the file, highlight the current Dynamic Outline heading.<br><span style="color: var(--text-accent)">Requires a restart to take effect.</span>`
				)
			)
			.addToggle((toggle) => {
				toggle
